...
## Step 3 — Set Up PostgreSQL (single instance + backups)

On a single VPS, **do not run replication** — all pods share the same machine so replication adds overhead without protecting against hardware failure. Instead: run a single PostgreSQL instance and rely on backups.
...
Create `deploy/saas/k8s/postgres.yaml`:
```yaml
...
metadata:
  ...
  namespace: sentinel
spec:
  instances: 1        # single instance on one VPS
  storage:
    size: 10Gi
  backup:
    barmanObjectStore:
      destinationPath: s3://your-bucket/sentinel-db   # or Hetzner Object Storage
      s3Credentials:
        accessKeyId:
  ...
  # Optional: scheduled daily backups
  scheduledBackups:
    - name: daily-backup
...
CloudNativePG exposes a service `sentinel-db-rw` — use it in `DATABASE_URL`:
```
DATABASE_URL=postgresql://sentinel:<password>@sentinel-db-rw:5432/sentinel
```

> **Note:** Add replication (`instances: 3`) only when you scale to multiple VPS nodes. On a single node it provides no availability benefit.
...
## Step 5 — Add Mercure Redis Transport
...
- [ ] Install CloudNativePG operator and create `postgres.yaml`
- [ ] Create S3/Hetzner Object Storage bucket for DB backups
- [ ] Add `S3_ACCESS_KEY` and `S3_SECRET_KEY` to cluster secrets
