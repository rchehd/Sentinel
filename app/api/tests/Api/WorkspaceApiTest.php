...
class WorkspaceApiTest extends WebTestCase
{
    ...
    public function testCreateWorkspaceSlugIsGeneratedFromName(): void
    {
        $client = static::createClient();
        $user = $this->createActiveUser(uniqid());
        $client->loginUser($user);
        ...
        $data = json_decode((string) $client->getResponse()->getContent(), true);
        $this->assertStringStartsWith('acme-corp-', $data['slug']);
    }

    public function testCreateWorkspaceWithReservedSlugFails(): void
    {
        $client = static::createClient();
        $user = $this->createActiveUser(uniqid());
        $client->loginUser($user);

        foreach (['login', 'register', 'admin', 'api'] as $reserved) {
            $client->request('POST', '/api/workspaces', [], [], [
                'CONTENT_TYPE' => 'application/json',
                'HTTP_ACCEPT' => 'application/json',
            ], (string) json_encode([
            ...
            $this->assertResponseStatusCodeSame(
                ...
                "Expected 422 for reserved slug '{$reserved}'",
            );
        }
    }

    public function testAutoGeneratedSlugSkipsReservedWords(): void
    {
        $client = static::createClient();
        $user = $this->createActiveUser(uniqid());
        $client->loginUser($user);
